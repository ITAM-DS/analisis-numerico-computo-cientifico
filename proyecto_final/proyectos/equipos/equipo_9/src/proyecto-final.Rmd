...
# Introducción
Uno de los objetivos de este proyecto es revisar algunos detalles e implementar 
la metodología de "aproximación por mínimos cuadrados Monte-Carlo" de Longstaff 
& Schwartz (2001) para la valuación de opciones Americanas. El objetivo de ésta 
...
+ Se dice que una opción tiene estilo **Bermuda** si sólo se puede ejercer 
...
mismo activo, con el mismo strike, la misma fecha de vencimiento y $r$ la 
tasa libre de riesgo. Entonces se  satisface una relación entre dichos precios 
que se conoce como la *paridad  put-call*, que se puede plantear en términos 
matemáticos como:
...
activo subyacente, $\delta$ es la tasa de dividendos continuos y $\Phi$ es la 
función de distribución de una variable aleatoria normal estándar.
	
	
# Valuación de opciones Americanas

Es difícil estimar el valor en situaciones donde la opción es afectada por 
más de un factor; es por esto que el método de diferencias finitas y binomial 
...
general, como “jump diffusions”; aunque la hipótesis más común es la de movimiento 
Browniano geométrico.

Funciona con semi martingalas. Las simulaciones tienen las siguientes características: 
simples, paralelizables,  transparentes y flexibles.  
...
se podrá obtener la especificación óptima para ejercer cada trayectoria.
		
En contraste investigaciones previas, desde Tilley (1993) hasta García (1999); 
no se utiliza Longstaff-Schwartz ya que proponen no utilizar varias estratificaciones 
o técnicas de parametrización para aproximar la función de continuación o acotar 
el ejercicio.
...
# Valuación de opciones Americanas vía la implementación de Longstaff & Schwartz
...
se toma sobre todos los tiempos de paro con respecto a la filtración, que se 
...
Americanas pueden ejercerse en cualquier punto en el tiempo, el algortimo
...
El valor de la opción se maximiza por escenarios si el inversionista ejerce 
...
$\mathbb{Q}$. Es decir, el valor de continuación al tiempo 
...
trayectorias de los cash-flows $E(\omega,s:t,T)$ generadas por la opción se 
definen de manera recursiva. Puede ocurrir que $E(\omega,s;t_j,T)$ sea diferente 
de $E(\omega,s;t_{j+1},T)$ ya que puede ser óptimo parar al tiempo $j_{j+1}$ y 
...
$V_c(\omega,t_{m-1})=\sum_{i=0}^{\infty}\beta_iL_i(S),$ donde $S$ es el valor del 
activo subyacente de la opción y los coeficientes $\beta_i$'s son constantes. 
...
$E(\omega,s;t_{m-1},T)$ con las funciones base como regresores en
...
idénticamente distribuidas a lo largo de los escenarios, no se
requieren suposiciones mayores para la existencia de los momentos y
por lo tanto, el valor ajustado de esta regresión,
...
**Observación:** esta descomposición existe en el caso de polinomios simples ya
que $A_j^{\top}A_j$ es una matriz de Vandermonde.
...
## Sobre la paralelización del algoritmo de Longstaff & Schwartz

### Paralelización del problema de optimización

Una mejora de optimización importante que hacen Longstaff & Schwartz (2001) es 
que sólo se considera en la regresión aquellos escenarios en los que el valor 
intrínseco (valor de la opción) es estrictamente positivo 
(i.e. la opción está *in-the-money*). Esto incrementa significativamente la 
eficiencia del algoritmo y disminuye el tiempo computacional.
...
La descomposición SVD suele ser computacionalmente costosa, mientras que  la 
sub-fase de evaluación del vector de valores de la opción al tiempo $t_j$ se 
hace utilizando los coeficientes que se calcularon en el tiempo $t_{j+1}$, 
por lo tanto esta parte es (por construcción) serial.

Sin embargo, una descomposición SVD hace algunas de las operaciones de manera 
independiente, hecho que hace que se puede llevar a cabo una paralelización 
efectiva. Por lo tanto la descomposición en valores singulares de cada una de 
las matrices de diseño, $A_j$, se puede llevar a cabo mediante una rutina en 
paralelo seguida de una actualización en serie de los vectores de valores de 
la opción $Y_j$.

Si $T$ es la longitud de cada una de las trayectorias y $c$ es el número de 
procesadores con los que se dispone, entonces cada una de las $T$ descomposiciones 
se hace de manera paralelizada con estos procesadores. 

Si $c$ no es tal que $T = c\cdot p$, i.e. $q$ no divide a $T$, se asignan más 
calculos a ciertos procesadores. La idea es sincronizar algunos de los cálculos 
SVD con las actualizaciones del vector de valores de la opción. Como los 
resultados de la actualización del valor de continuación se obtienen de orden 
de los mejores procesadores (los poco ocupados) a los peores procesadores 
(los muy ocupados), y el algoritmo completo es intrinsecamente serial, los 
peores procesadores pasarán ocupados con cálculos para la SVD durante este tiempo.

En la fase de calibración, después de que se llevó a cabo la descomposición en 
valores singulares, el mejor procesador calculará las actualizaciones para los 
tiempos de simulación correspondienes, i.e. $T-2, T-3,\ldots,T-\lfloor T/q\rfloor$. 
Esto envía el vector de valores de la opción para $T-\lfloor T/q\rfloor$ al 
siguiente mejor procesador. Usando este vector de valores, el siguiente procesador 
calcula los de los tiempos $T-\lfloor T/q\rfloor-1,\ldots,T-2\lfloor T/q\rfloor$.
...
Dicha descarga y simulaciones se implementan en la rutina del archivo:  *[rutina_1.R](https://github.com/philwebsurfer/analisis-numerico-computo-cientifico/blob/mno-2019-1/proyecto_final/proyectos/equipos/equipo_9/src/mno_regresiones_simul.Rmd)*

## Estimación de parámetros e implementación de paro

La implementación numérica se lleva a cabo en el script de R el cual se encuentra en la siguiente liga:  
*[- Rutinas de Optimización](https://github.com/philwebsurfer/analisis-numerico-computo-cientifico/blob/mno-2019-1/proyecto_final/proyectos/equipos/equipo_9/src/mno_regresiones.R)*



## Algunos resultados de la implementación

Se tomó un histórico de 180 días para estimar los parámetros que se utilizaron 
en la generación de las 100,00 trayectorias de los precios de cada activo. A 
continuación se muestran dichos históricos de precios:
...
A continución se muestran algunos resultados de la implementación para una *put* 
con activo subyacente una acción de Intel con precio strike $K = 49$
```{r, warning=FALSE, message=FALSE, echo=FALSE, cache=TRUE}
...
Se puede observar que la mayoría de las estrategias de ejercicio establecen 
una salida anticipada durante los primeros 5 días del contrato.  

También se puede observar que la dependencia significactiva más grande se obtuvo 
con covariables de hasta la potencia cúbica de los precios.

#### Call

A continución se muestran algunos resultados de la implementación para una *call* 
con activo subyacente una acción de Intel con precio strike $K = 49$
...
Se puede observar que la mayoría de las estrategias de ejercicio establecen 
una salida anticipada durante los primeros 10 días del contrato.  
...
A continución se muestran algunos resultados de la implementación para una *put* 
con activo subyacente una acción de HP con precio strike $K = 14$
...
Se puede observar que la mayoría de las estrategias de ejercicio establecen 
una salida anticipada durante los primeros 12 días del contrato.  

También se puede observar qeu la dependencia significactiva más grande se obtuvo 
con covariables de hasta la potencia cuarta de los precios.
...
A continución se muestran algunos resultados de la implementación para una *put* 
con activo subyacente una acción de IBM con precio strike $K = 124$
...
También se puede observar que la dependencia significactiva más grande se obtuvo 
con covariables de hasta la potencia cuadrada de los precios pero hay casos con 
dependencia de hasta grado 8.
...
Se puede observar que la mayoría de las estrategias de ejercicio establecen 
una salida anticipada durante los primeros 14 días del contrato.  

También se puede observar que la dependencia significactiva más grande se obtuvo 
con covariables de hasta la potencia cuadrada de los precios.
...
Ryan, J., Ulrich, J., Thielen, W. Teetor, P. & Bronder, S. (2007). Package *quantmod*. Quantitative Financial Modelling Framework 
