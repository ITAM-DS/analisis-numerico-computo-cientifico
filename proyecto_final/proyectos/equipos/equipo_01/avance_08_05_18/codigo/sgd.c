...
double sgd(int M, int N, int M_v, int batch_size, int iter, double lr, FILE *f) {
...
fprintf(f, "%d,%d, %lf, %lf, %e \n", it, iter, acum/M, acum_v/M_v, lr);
...
int main(int argc, char const *argv[]) {
    ...
    // double lrs[2] = {-0.0000001, -0.000001};

    double loss[10] = {0};
    
    fprintf(f2, "learning_rate, loss\n");
    fprintf(f, "iteracion, tot_iter, rss_train, rss_val, lr\n");
    for(int i = 0; i<sizeof(lrs)/sizeof(lrs[0]); i++){
        loss[i] = sgd(M, N, M_v, batch_size, iter, lrs[i],f);
    }
