...
for (i=0;i<row-c_actual; i++){
	...
	for (j=0;j<row-c_actual; j++){
		...
		acumula+=((I_Bvvt[i][j])*vectorc[j]);
		printf(" I_Bvvt:  %f",I_Bvvt[i][j]);
		printf(" vectorc[j] %d",vectorc[j]);
		//[k][c_actual]+=I_Bvvt[i][j]*vectorc[i];	
	}
	*(arr + ((i+c_actual)*col)+c_actual)=acumula;
