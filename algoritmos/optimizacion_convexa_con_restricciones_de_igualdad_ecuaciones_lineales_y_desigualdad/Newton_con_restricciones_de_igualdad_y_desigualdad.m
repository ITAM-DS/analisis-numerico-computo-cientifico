...
function [x,iter,Err_plot,x_plot]=Newton_con_restricciones_de_igualdad(f,t_path,f_rest,A,b,x_ast,p_ast,x0,tol,tol_backtracking,maxiter)
%Entrada:
...
if(valor_restriccion<tol)
	...
	%Evaluaciones:
	feval=f(x);
	f_barrera_log_eval = Barrera_logaritmica(f,f_rest,t_path,x);

	gfeval = t_path*Gradiente_f(f,x)+Gradiente_f_rest(f_rest,x);

	Hfeval = t_path*Hessiana_f_dif_ad(f,x) + Hessiana_f_rest(f_rest,x);
	...
	%Modificar siguientes líneas para evitar crear matriz_sistema en el caso A no ceros
	...
	fprintf('Iter    Norma(tgf+phi)        f(x)            tf+phi            Decremento_en_x     Error_x_ast      Error_p_ast      backtracking     Condición_matriz_sistema\n')
	
	fprintf('%3i    %1.7e      %1.7e      %1.7e      %1.7e      %1.7e      %1.7e      %s              %1.7e\n',iter, normagf, feval, f_barrera_log_eval, lambda_cuadrada_Newton, Err, Err_plot_aux(iter),'---', cond_matriz_sistema);
	...
	while(criterio_de_paro>tol && iter<maxiter)
		...
		t=Metodo_backtracking(alpha, beta,f,f_rest,dir_Newton,t_path,x,derivada_direccional_Newton);
		x=x+t*dir_Newton;
		...
		fprintf('%3i    %1.7e      %1.7e      %1.7e      %1.7e      %1.7e      %1.7e      %1.7e      %1.7e\n',iter, normagf, feval, f_barrera_log_eval, lambda_cuadrada_Newton, Err, Err_plot_aux(iter),t, cond_matriz_sistema);
