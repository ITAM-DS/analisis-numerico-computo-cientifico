from utils_2nd_version import norm_residual, logarithmic_barrier
...
def line_search_for_log_barrier_by_backtracking(f,dir_desc,x,t_path,
    ...
    if t!=-1:
        if not infeasible:
            ...
            eval1 = logarithmic_barrier(f,x + t*dir_desc, t_path,constraint_inequalities, infeasible=infeasible)
            eval2 = logarithmic_barrier(f,x, t_path,constraint_inequalities, infeasible=infeasible) + alpha*t*der_direct            
        while eval1 > eval2:
            t=beta*t
            if not infeasible:
                eval1=logarithmic_barrier(f,x + t*dir_desc, t_path,constraint_inequalities)
