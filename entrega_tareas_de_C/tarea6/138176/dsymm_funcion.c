...
#define A_matriz "A.txt" //simétrica de tamaño MxM
...
int main(int argc, char *argv[]){
  ...
  /* Los parámetros "L" y "L" implican que la la multiplicación será A*B y que de la matriz A se
  referenciaran los valores en el triangulo inferior de la matriz, dado que es simétrica */
	dsymm_("L", "L", &M, &N, &ALPHA, entradas(A), &M, entradas(B), &M, &BETA, entradas(C), &M);
	imprime_matriz(C);
	free(entradas(A));
	free(A);
	free(entradas(B));
