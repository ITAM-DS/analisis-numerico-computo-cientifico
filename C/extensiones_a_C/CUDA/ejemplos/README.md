...
Debajo del directorio de `samples` por ejemplo: `/Developer/NVIDIA/CUDA-7.5/samples/` se tienen los niveles `1_Utilities/deviceQuery` que al hacer el `make` que se encuentra en este último nivel se obtiene el ejecutable `deviceQuery` cuyo output nos da información del device: 
...
deviceQuery, CUDA Driver = CUDART, CUDA Driver Version = 7.5, CUDA Runtime Version = 7.5, NumDevs = 1, Device0 = GeForce GTX 780M
Result = PASS
```
